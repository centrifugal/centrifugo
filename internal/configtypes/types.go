...
type ProxyCommon struct {
	...
	// ClientEmulatedHeaders is a list of client-supplied emulated header names to proxy. None by default.
	ClientEmulatedHeaders []string `mapstructure:"client_emulated_headers" json:"client_emulated_headers" envconfig:"client_emulated_headers" yaml:"client_emulated_headers" toml:"client_emulated_headers" expose:"full" doc:"List of header names taken from the client headers emulation map (sent inside the client's connect frame) to forward to the proxy backend. These values are controlled by the client, so only list names your backend independently validates as untrusted client input (e.g. a signed token) - never names you trust by origin, those belong in http_headers only. A name may also appear in http_headers (e.g. an Authorization token browsers send via emulation and native clients send as a real header): the transport value is preferred and the emulated value used as a fallback."`
	// HttpHeadersIncludeClientEmulated restores the behavior before v6.9.0 where names in HttpHeaders
	// were also sourced from the client emulation map. Deprecated, off by default, will be removed in v7.
	HttpHeadersIncludeClientEmulated bool `mapstructure:"http_headers_include_client_emulated" json:"http_headers_include_client_emulated" envconfig:"http_headers_include_client_emulated" yaml:"http_headers_include_client_emulated" toml:"http_headers_include_client_emulated" doc:"Deprecated, will be removed in v7. When enabled, header names in http_headers are also forwarded from the client headers emulation map (the behavior before v6.9.0), making them client-forgeable. Off by default - prefer listing client-supplied headers in client_emulated_headers."`

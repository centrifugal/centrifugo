...
func TestBadMethod(t *testing.T) {
	s := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		ws, _, err := cstUpgrader.Upgrade(w, r, nil)
		if err == nil {
...
func TestHandshake(t *testing.T) {
	...
	ws, resp, subprotocol, err := cstDialer.Dial(s.URL, http.Header{"Origin": {s.URL}})
	if err != nil {
		t.Fatalf("Dial: %v", err)
	}
	defer func() {
	...
	if subprotocol != "p1" {
		t.Errorf("ws.Subprotocol() = %s, want p1", subprotocol)
	}
	sendRecv(t, ws)
}
...
func TestHost(t *testing.T) {
	...
	handler := http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if IsWebSocketUpgrade(r) {
			c, _, err := upgrader.Upgrade(w, r, http.Header{"X-Test-Host": {r.Host}})
			if err != nil {
				t.Fatal(err)
			}
			_ = c.Close()
	...
	for i, tt := range tests {
		...
		c, resp, _, err := dialer.Dial(wsProtos[tt.server]+tt.url+"/", h)
		if err == nil {
			_ = c.Close()
		}

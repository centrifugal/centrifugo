...
const (
	...
	CentrifugoProxy_LoadDocuments_FullMethodName           = "/centrifugal.centrifugo.proxy.CentrifugoProxy/LoadDocuments"
...
type CentrifugoProxyClient interface {
	...
	// LoadDocuments is an EXPERIMENTAL method which allows to load documents from the backend.
	LoadDocuments(ctx context.Context, in *LoadDocumentsRequest, opts ...grpc.CallOption) (*LoadDocumentsResponse, error)
...
func (c *centrifugoProxyClient) LoadDocuments(ctx context.Context, in *LoadDocumentsRequest, opts ...grpc.CallOption) (*LoadDocumentsResponse, error) {
	out := new(LoadDocumentsResponse)
	err := c.cc.Invoke(ctx, CentrifugoProxy_LoadDocuments_FullMethodName, in, out, opts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
type CentrifugoProxyServer interface {
	...
	// LoadDocuments is an EXPERIMENTAL method which allows to load documents from the backend.
	LoadDocuments(context.Context, *LoadDocumentsRequest) (*LoadDocumentsResponse, error)
	// NotifyChannelState can be used to receive channel events such as channel "occupied" and "vacated".
	// This is a feature in a preview state and is only available in Centrifugo PRO.
...
func (UnimplementedCentrifugoProxyServer) LoadDocuments(context.Context, *LoadDocumentsRequest) (*LoadDocumentsResponse, error) {
	return nil, status.Errorf(codes.Unimplemented, "method LoadDocuments not implemented")
}
...
func _CentrifugoProxy_LoadDocuments_Handler(srv interface{}, ctx context.Context, dec func(interface{}) error, interceptor grpc.UnaryServerInterceptor) (interface{}, error) {
	in := new(LoadDocumentsRequest)
	if err := dec(in); err != nil {
		return nil, err
	}
	if interceptor == nil {
		return srv.(CentrifugoProxyServer).LoadDocuments(ctx, in)
	}
	info := &grpc.UnaryServerInfo{
		Server:     srv,
		FullMethod: CentrifugoProxy_LoadDocuments_FullMethodName,
	}
	handler := func(ctx context.Context, req interface{}) (interface{}, error) {
		return srv.(CentrifugoProxyServer).LoadDocuments(ctx, req.(*LoadDocumentsRequest))
	}
	return interceptor(ctx, in, info, handler)
}
...
var CentrifugoProxy_ServiceDesc = grpc.ServiceDesc{
	...
	Methods: []grpc.MethodDesc{
		...
		{
			MethodName: "LoadDocuments",
			Handler:    _CentrifugoProxy_LoadDocuments_Handler,
		},
		{

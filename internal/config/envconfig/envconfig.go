	...
	for _, info := range infos {
		...
		if !ok && def == "" {
			if isTrue(req) {
				...
				return nil, fmt.Errorf("required key %s missing value", key)
			}
			continue
		}

		err = processField(value, info.Field, defaultIsUsed)
		if err != nil {
...
func processField(value string, field reflect.Value, defaultIsUsed bool) error {
	...
	case reflect.Slice:
		...
		if typ.Elem().Kind() == reflect.Uint8 {
			...
			vals := strings.Split(value, SliceSep)
			sl = reflect.MakeSlice(typ, len(vals), len(vals))
			for i, val := range vals {
				err := processField(val, sl.Index(i), defaultIsUsed)
				if err != nil {
					return err
				}
			}
	...
	case reflect.Map:
		...
		if strings.TrimSpace(value) != "" {
			...
			for _, pair := range pairs {
				...
				err := processField(kvpair[0], k, defaultIsUsed)
				if err != nil {
					return err
				}
				v := reflect.New(typ.Elem()).Elem()
				err = processField(kvpair[1], v, defaultIsUsed)
				if err != nil {
					return err
				}
				mp.SetMapIndex(k, v)

-- Auto-generated unified schema — do not edit.
-- Apply this file for fresh installs. For upgrades, see migrations/.
...
BEGIN
    ...
    IF p_num_shards IS NULL THEN
        SELECT COUNT(*)::INTEGER INTO p_num_shards FROM cf_map_shard_lock;
    END IF;
    ...
    IF NOT p_skip_shard_lock THEN
        PERFORM 1 FROM cf_map_shard_lock WHERE shard_id = v_shard_id FOR UPDATE;
    END IF;
...
BEGIN
    ...
    LOOP
        ...
        DELETE FROM cf_map_state WHERE channel = v_channel AND key = ANY(v_keys);
...
-- cf_map_publish_stream: Stream-only publish for ExternalState mode.
-- Skips state table entirely — only writes to stream + meta.
CREATE OR REPLACE FUNCTION cf_map_publish_stream(
    p_channel TEXT,
    p_key TEXT,
    p_data JSONB,
    p_tags JSONB DEFAULT NULL,
...
-- cf_map_remove_stream: Stream-only remove for ExternalState mode.
-- Skips state table entirely — only writes to stream + meta.
CREATE OR REPLACE FUNCTION cf_map_remove_stream(
    p_channel TEXT,
    p_key TEXT,
    p_client_id TEXT DEFAULT NULL,
    p_user_id TEXT DEFAULT NULL,
...
-- cf_map_stream_top_position: Get current stream top position (top_offset + epoch).
...
CREATE OR REPLACE FUNCTION cf_map_stream_top_position(
    p_channel TEXT
) RETURNS TABLE(
    top_offset BIGINT,
    epoch TEXT
...
BEGIN
    ...
    LEFT JOIN cf_map_meta m ON m.channel = p_channel;
END;
$$ LANGUAGE plpgsql;
...
CREATE TABLE IF NOT EXISTS cf_binary_map_shard_lock (
    shard_id SMALLINT PRIMARY KEY
);
...
CREATE TABLE IF NOT EXISTS cf_binary_map_schema_version (
    id              INTEGER PRIMARY KEY,
    schema_version  INTEGER NOT NULL
);
INSERT INTO cf_binary_map_schema_version (id, schema_version) VALUES (1, 1)
    ON CONFLICT (id) DO NOTHING;
